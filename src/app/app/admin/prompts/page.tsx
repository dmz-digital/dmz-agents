...
import {
    MessageSquare, ArrowLeft, Save,
    RotateCcw, Sparkles, AlertCircle,
    CheckCircle2, Loader2, Bot,
    Wand2, Search, Code2, FileText,
...
const SYSTEM_PROMPTS = [
    ...
    { id: "tool_create_image", label: "Gerador de Imagem (Nano Banana)", category: "Ferramentas", icon: ImageIcon },
    { id: "tool_search_web", label: "Deep Web Research (Firecrawl)", category: "Ferramentas", icon: Globe },
...
export default function PromptsAdmin() {
    ...
    async function loadPrompts() {
        ...
        if (data) {
            ...
            data.forEach(p => map[p.agent_id] = p.content);

            // Fill missing prompts with defaults
    ...
    async function handleSave() {
        ...
        try {
            const content = prompts[selectedId];
            ...
            setOriginalPrompts(prev => ({ ...prev, [selectedId]: content }));
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        <div className="bg-white border border-neutral-100 rounded-[32px] p-4 flex flex-col gap-1">
                            {["Chat", "Ferramentas", "Anexos"].map(category => (
                                <div key={category} className="mb-4 last:mb-0">
                                    <div className="px-4 py-2 text-[10px] font-black text-neutral-300 uppercase tracking-widest">{category}</div>
                        ...
                        <div className="p-6 bg-neutral-900 rounded-[32px] text-white">
                            ...
                            <p className="text-[11px] leading-relaxed text-white/60 font-medium">
                                Use variáveis como {"{user_message}"} para referenciar o contexto se necessário. O Orchestrator já inclui o contexto do projeto automaticamente.
                            </p>
                        </div>
                    </div>
                        ...
                        <div className="bg-white border border-neutral-100 rounded-[40px] p-10 shadow-sm relative overflow-hidden">
                            <div className="flex items-center justify-between mb-8">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 bg-neutral-50 rounded-2xl flex items-center justify-center text-neutral-900">
                                        {SYSTEM_PROMPTS.find(p => p.id === selectedId)?.icon({ size: 24 })}
                                    </div>
                                    <div>
                                        <h2 className="text-xl font-black text-neutral-900">{SYSTEM_PROMPTS.find(p => p.id === selectedId)?.label}</h2>
                                ...
                                <div className="flex items-center gap-3">
                                    <button
                                        ...
                                        className="p-3 hover:bg-neutral-50 rounded-2xl text-neutral-400 transition-all disabled:opacity-30"
                                    ...
                                    <button
                                        onClick={handleSave}
                                        disabled={!hasChanged || saving}
                                        className="bg-dmz-accent text-white px-8 py-3.5 rounded-2xl font-bold text-sm shadow-xl shadow-dmz-accent/20 hover:bg-orange-600 transition-all disabled:opacity-50 flex items-center gap-2"
                                    >
                            ...
                            <div className="relative">
                                <textarea
                                    ...
                                    className="w-full h-[500px] p-8 bg-[#F9FAFB] border border-neutral-100 rounded-3xl text-sm font-medium leading-relaxed resize-none focus:ring-0 focus:border-dmz-accent/20 transition-all custom-scrollbar outline-none text-neutral-800"
                                />
                                {hasChanged && !saving && (
                            ...
                            <AnimatePresence>
                                {message && (
                                    <motion.div
                                        ...
                                        exit={{ opacity: 0 }}
                                        className={`mt-6 p-4 rounded-2xl flex items-center gap-3 ${message.type === 'success' ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-red-50 text-red-700 border border-red-100'}`}
                                    >
