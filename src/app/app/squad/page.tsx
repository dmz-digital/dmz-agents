...
function AgentIcon({ agentId, size = 18, color = "#6B7280" }: { agentId: string; size?: number; color?: string }) {
...
function Tag({ children, color, small }: { children: React.ReactNode; color: string; small?: boolean }) {
    return (
        <span style={{
...
function Handle({ handle, color }: { handle: string; color: string }) {
    return (
        <div style={{
...
function LevelBadge({ level }: { level: string }) {
    const colors: Record<string, string> = { L0: "#E85D2F", L1: "#7C3AED", L2: "#6B7280" };
    const labels: Record<string, string> = { L0: "Maestro", L1: "Lead", L2: "Specialist" };
            ...
            {tab === "protocolo" && (
                <div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "0" }}>
                        {agent.protocols[0].split(" → ").map((step: string, i: number, arr: string[]) => (
                            <div key={i}>
                                <div style={{
                                    display: "flex", alignItems: "center", gap: "10px",
                                    padding: "10px 14px",
...
function CategoryBreakdown({ activeFilter, onFilter }: { activeFilter: string | null; onFilter: (f: string | null) => void }) {
    return (
...
function HierarchyView() {
    ...
    const levelColors: Record<string, string> = { "L0 — Maestro": "#E85D2F", "L1 — Leads": "#7C3AED", "L2 — Specialists": "#6B7280" };
...
export default function SquadAboutPage() {
    ...
    return (
        <div style={{ background: "#F7F7F5", fontFamily: "'Plus Jakarta Sans', 'Segoe UI', sans-serif" }} className="px-10 py-12">
            {/* Internal View Toggle (Local Nav) */}
            <div style={{
                display: "flex",
                gap: "10px",
                marginBottom: "32px",
