...
echo ""
echo -e "${CYAN}Instalando/Atualizando dmz-os v0.4.3 (MCP Universal)...${RESET}"
...
if ! $PIP install "$INSTALL_CMD" --quiet --upgrade --no-cache-dir 2>/dev/null; then
    if ! $PIP install "$INSTALL_CMD" --quiet --upgrade --no-cache-dir --break-system-packages 2>/dev/null; then
        echo -e "${YELLOW}  ⚠ Global install falhou. Iniciando Auto-Healing (Virtual Environment)...${RESET}"
        mkdir -p .agents
        python3 -m venv .agents/.venv
        source .agents/.venv/bin/activate
        pip install "$INSTALL_CMD" --quiet --upgrade --no-cache-dir
    fi
fi

# Pega o executável correto
DMZ_CMD="dmz-os"
if [ -f ".agents/.venv/bin/dmz-os" ]; then
    DMZ_CMD=".agents/.venv/bin/dmz-os"
elif ! command -v dmz-os &>/dev/null; then
    ...
    if ! command -v dmz-os &>/dev/null && ! python3 -c "import dmz_os" &>/dev/null 2>&1; then
        echo -e "${RED}✗ Falha crítica: não foi possível instalar o DMZ OS.${RESET}"
        echo -e "${DIM}  Por favor, contate o suporte ou verifique suas permissões.${RESET}"
        exit 1
    fi
    DMZ_CMD="python3 -m dmz_os"
fi
